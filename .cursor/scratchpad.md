...
## Key Challenges and Analysis (2026-08-28 이미지 2안 방법)

사용자: 2안 분석 방법과 단어 목록을 넣는 방법을 물어봄. Planner만. 구현 없음.

분석: CLIP(ONNX, 이 PC). 미리 넣은 한국어 사전과 사진 특징을 비교. 점수 높은 단어만 고름. YOLO는 보조가 아님(기본 아님). 클라우드 캡션 안 함.

저장: 새 엔진 없음. `documents.body_text`에 공백으로 이은 한국어 단어. 1안 OCR과 이어 붙임. 검색은 기존 `body_text` LIKE. 이미지 파일명은 매칭에서 빼는 쪽.

사전은 앱에 고정. 사용자가 태그 편집하지 않음.

정확도: 큰 물체 대략 60%대(영어 공개 시험). 세부는 약함. 한국어 직접 비교는 거의 실패 → 영어 프롬프트, 저장은 한국어.
크기: 지금 Setup 67MB. 작은 CLIP +150~250MB → 설치 220~320MB. 큰 모델은 1GB 전후.
속도: 검색은 기존. 인덱싱 장당 0.3~2초(CPU, 작은 모델). 200장 1~10분.

## Background and Motivation (2026-08-31 기능 고도화)

사용자: 기능 고도화 제안을 요청. Planner만. 구현 없음.

이미 된 것: 본문·OCR, 종류 여러 개, 검색 1안, 정보 1안, 업데이트 팝업(0.1.29).
이미 제안만: 이미지 1·2안, 보안 1·2·3안, 검색 UI 2·3안.

## High-level Task Breakdown (기능 고도화, 미승인)

고도화 1안(추천): 올해/최근 1년, 최근 검색어 칩, 결과 최신순.
고도화 2안: 처음 실행 「이 폴더 읽기」+ 진행 막대.
고도화 3안: 건너뛴·실패 파일 건수 보이기.
고도화 4안: 사진 OCR. 5안: PPT. 6안: CLIP.
보안 1안은 같이 고를 수 있음.
...
## Background and Motivation (2026-08-31 기능 고도화 2차)

사용자: 기능 제안 더. Planner만. 구현 없음. 5안 PPT는 사용자 확인 완료.

이미 제안: 1~4·6안, 보안, CLIP. 이번은 겹치지 않는 2차.

## High-level Task Breakdown (기능 고도화 2차, 미승인)

7안: 파일 한 줄 메모(원본 안 고침). 이름 없는 사진에 정확함.
8안: 같은 내용 복사본 묶기.
9안: 짧은 동의어(계약/계약서).
10안: 숫자·금액 정규화.
11안: .eml 본문. .msg는 나중.
...
추천: 매일은 1안. 사진 글자면 4안, 내가 알면 7안, 장면 자동은 4+6.

## Background and Motivation (2026-08-31 기능 고도화 3차)

사용자: 다른 제안은? Planner만. 1~12와 겹치지 않게.

## High-level Task Breakdown (기능 고도화 3차, 미승인)

13안: 바로 아래 폴더 필터.
14안: 최근 열기.
15안: 결과 파일명 복사.
16안: 하위 폴더 제외.
17안: OneDrive 자리표시 안내.
18안: 인덱싱 중 검색.
19안: HEIC OCR.
20안: PDF/PPT 근거 쪽 번호.
21안: 본문 없는 파일만.
